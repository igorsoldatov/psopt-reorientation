...
adouble get_constraints(adouble x1, adouble x2, adouble x3, adouble cos, adouble q1, adouble q2, adouble q3, adouble q4){
    // x = [x1, x2, x3]
    // y = [y1, y2, y3] = [1, 0, 0]
    // angle = 40 grad
    // cos = cos(angle)
    ...
    */
    ...
    adouble m[] = {q1 * m1 + q2 * m5 + q3 * m9 + q4 * m13,
                   q1 * m2 + q2 * m6 + q3 * m10 + q4 * m14,
                   q1 * m3 + q2 * m7 + q3 * m11 + q4 * m15,
                   q1 * m4 + q2 * m8 + q3 * m12 + q4 * m16};

    return m[0] * q1 + m[1] * q2 + m[2] * q3 + m[3] * q4;
}
         ...
         adouble* xad, int iphase, Workspace* workspace)
{
    ...
    */
    ...
    //adouble x2 = 3.39;
    //adouble x3 = -3.49;
    //adouble x1 = 0.3368242;
    //adouble x2 = 0.1736482;
    //adouble x3 = -0.9254165;
    adouble cos = 0.525322;

    adouble value1 = get_constraints(0.3368242, 0.1736482, -0.9254165, cos, q1, q2, q3, q4);
    adouble value2 = get_constraints(-0.05238328, -0.9882016, 0.1439218, cos, q1, q2, q3, q4);
    //adouble value3 = get_constraints(-0.1418878, 0.909889, 0.3898333, cos, q1, q2, q3, q4);
    //path[ 0 ] = value1;
    //std::cout << "value1 = " << value1 << ", " << q1 << ", " << q2 << ", " << q3 << ", " << q4 << std::endl;
...
int reorientation_task(int nodes, int end_time)
{
...
////////////////////////////////////////////////////////////////////////////
    ...
    const auto nodes_str = std::to_string(nodes);
    const auto end_time_str = std::to_string(end_time);
    const auto task = std::to_string(nodes) + "-" + std::to_string(end_time) + "-";

    problem.name        		= "Reorientation, params: " + nodes_str + "/" + end_time_str; // "Reorientation of a rigid body"
    problem.outfilename         = task + "reorientation.txt";
...
////////////////////////////////////////////////////////////////////////////
    ...
    problem.phases(1).bounds.lower.states << -1.0,    -1.0,    -1.0,   -0.3,    -0.3,  -0.3;
    problem.phases(1).bounds.upper.states <<  1.0,     1.0,     1.0,    0.3,     0.3,   0.3;

    problem.phases(1).bounds.lower.controls << -2.0,  -2.0, -2.0,  -1.0;
    problem.phases(1).bounds.upper.controls <<  2.0,   2.0,  2.0,   1.0;

    problem.phases(1).bounds.lower.path(0) = 0.000;
    problem.phases(1).bounds.upper.path(0) = 0.000;
    problem.phases(1).bounds.lower.path(1) = -1000000.000;
    problem.phases(1).bounds.upper.path(1) = 0.000;
    problem.phases(1).bounds.lower.path(2) = -1000000.000;
    problem.phases(1).bounds.upper.path(2) = 0.000;
    //problem.phases(1).bounds.lower.path(3) = -1000000.000;
    //problem.phases(1).bounds.upper.path(3) = 0.000;

    MatrixXd q0(4,1), qf(4,1), omega0(3,1), omegaf(3,1);

    //  0.7143, 0.6365, -0.1300, 0.2602
    q0(0) = 0;
    q0(1) = 0;
    q0(2) = 0;
    q0(3) = 1;
    // 0.2297, -0.0081, -0.4909, 0.8404
    qf(0) = 0;
    qf(1) = 1;
    qf(2) = 0;
    qf(3) = 0;
    ...
    problem.phases(1).bounds.upper.EndTime      = (double)end_time;
...
////////////////////////////////////////////////////////////////////////////
    ...
    Save(states, (task + std::string("states.dat")).c_str());
    Save(controls, (task + std::string("controls.dat")).c_str());
    Save(t, (task + std::string("t.dat")).c_str());
...
////////////////////////////////////////////////////////////////////////////
    ...
    multiplot(t,q,problem.name+", quarternion","time (s)", "q1 q2 q3 q4", "q1 q2 q3 q4", 2, 2,
                "jpeg", (task + std::string("reorientation_q.jpeg")).c_str());

    multiplot(t,u,problem.name+", controls", "time (s)", "u1 u2 u3", "u1 u2 u3", 3, 1,
               "jpeg", (task + std::string("reorientation_u.jpeg")).c_str());
